...
public class Knife : FinishProduct {

    private const int IRON_QUANTITY = 1;
    private const int WOOD_QUANTITY = 1;
    ...
    public override bool Is_conform_to_component_quantity(string component_name)
    {
        if (component_name == "wood")
            ...
            iron_count++;
        if (wood_count >= WOOD_QUANTITY && iron_count >= IRON_QUANTITY)
        {
    ...
    {
        Set_image(image_manager.Get_finish_product_image((int)FinishProductImage.knife));
        Set_components();
        Reset_component_quantity();
    }

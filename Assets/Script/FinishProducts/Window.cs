...
public class Window : FinishProduct {

    private const int GLASS_QUANTITY = 2;
    private const int IRON_QUANTITY = 4;

    private ImageManager image_manager;
    private int glass_count;
    private int iron_count;

    public Window(ImageManager manager)
    {
    ...
    {
        Set_image(image_manager.Get_finish_product_image((int)FinishProductImage.window));
        Set_components();
        Reset_component_quantity();
    }
